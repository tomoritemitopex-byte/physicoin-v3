  ...
  useEffect(() => {
    try {
      ...
      fetch("/api/mining?round=current", { cache: "no-store" })
        ...
        .then((current) => { if (current.ok) setRound(current); })
        .catch(() => setMessage("Could not load the active round."));
      if (id) load(id).catch(() => setMessage("Could not load your mining records."));
    } catch {
        ...
        <div className="slip flex flex-col gap-5 p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-black">{round ? `Round ${round.round}` : "Loading round..."}</h2>
            </div>
            <span className="rounded-full bg-sky-deep px-3 py-1 font-mono text-[11px] font-bold text-accent">{round?.status || "syncing"}</span>
          </div>
          ...
          <button onClick={mine} disabled={busy || !round} className="rounded-full bg-forest px-5 py-3.5 text-sm font-black text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50">
            {busy ? "Grinding proof..." : "Mine this round"}
          </button>
          <p aria-live="polite" className="min-h-5 font-mono text-xs text-ink/65">{message}</p>
        </div>
