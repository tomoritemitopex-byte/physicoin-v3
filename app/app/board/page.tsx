...
/* Hall heat sparkline — Bend-emitted JS (conceptual)
   This heat calc *could* be generated by Bend-emitted JS from bend/Emit.bend:
     import { hall_heat } from "@/bend/Emit.js"  // produced by `bend build bend/Emit.bend -o /tmp/emit.js` via scripts/bend-emit.sh
     // hall_heat(count: Nat) -> Nat returns count * 2  (Bend parallel: each hall's count is doubled in parallel)
   Current JS implementation below is the fallback and stays active — additive, no new tables.
   Keep JS fallback until Bend JS import is wired; see scripts/bend-emit.sh and bend/Emit.bend. */
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky/30 via-[#f0fbff] to-white pb-16 text-ink">
      ...
      <div className="mx-auto max-w-[1120px] px-4 pt-6 sm:px-6">
        ...
        {/* HEAT HALL — one-glance pending heat
            Bend conceptual: hall heat sparkline could be generated by Bend-emitted JS
            (bend/Emit.bend → hall_heat(count) = count*2, compiled via `bend build -o /tmp/emit.js`).
            Current JS (heatFromSlips + serverHeat) is the fallback — keep until import wired.
            See bend/Emit.bend and scripts/bend-emit.sh; warn-not-fail if bend not installed. */}
        {/* HEAT HALL — one-glance pending heat */}
        {(() => {
